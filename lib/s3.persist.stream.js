...
function S3PersistStream(options) {
  ...
  this.s3 = new AWS.S3();
  this.s3.config.credentials = awsCredentials;
  ...
  if (!options.maindir) {
    throw new Error('maindir must be specified for S3 persist stream');
  }

  if (!options.subdir) {
    throw new Error('subdir must be specified for S3 persist stream');
  }
  ...
  this._maindir = options.maindir;
  this._subdir = options.subdir;
  this._bucket = options.bucket;

  this._uploadParams = {
    "Bucket": this._bucket,
    "Key": util.format('%s/%s/%s', this._maindir, this._subdir, this._filename),
    "StorageClass": "REDUCED_REDUNDANCY"
  }
  this._listObjectParams = {Bucket: this._bucket,
                            Prefix: util.format('%s/%s/', this._maindir, this._subdir)};
...
S3PersistStream.prototype.rotate = function () {
  var self = this;
  this.s3.listObjects(this._listObjectParams, function (err,data) {
    if (!err) {
      var contents = data.Contents;
      contents.map(function (item) { _.forEach(item, function(i,j) {if (j !== "Key") delete item[j]; });});
      var toDelete = contents.filter(function(item) { return parseInt(item.Key.split('/')[2]) <= self._filename - rotateHistoryCount; });
